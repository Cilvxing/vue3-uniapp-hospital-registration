    ...
    <view class="homepage-back">
      <view class="doctor-int">
        ...
        <view>
          ...
          <text>遵义医科大学附属医院</text>
        </view>
      </view>
      <view class="good-at">{{item.good_at}}</view>
      <!-- <view class="good-at">擅长：创伤骨科各病种的诊断及常规治疗，熟悉四肢及盆骨髋臼骨折的微创治疗</view> -->
    </view>
    <!-- 选择时间 -->
    <view class="homepage-back">
      ...
      <view class="table">
        ...
        <scroll-view scroll-x="true" class="scroll-view_H" enable-flex="true">
          <view class="TR" v-for="(item_a, index_a) in item.App_ment" :key="index_a">
            ...
            <!-- 0:下午 -->
            <view :class="[item_a.time[0].nu_source <= 0 ? 'Disa' : 'Have']" @click="selectTime(item_a.week, item_a.time[0].the_time, item_a.time[0].when)"
            >
              {{item_a.time[0].nu_source <= 0 ? '' : '预约'}}
            </view>
            <!-- 1:下午 -->
            <view :class="[item_a.time[1].nu_source <= 0 ? 'Disa' : 'Have']" @click="selectTime(item_a.week, item_a.time[1].the_time, item_a.time[1].when)"
            >
...
import {Doctorhome} from '@/public/decl-type'
// 预约弹窗组件
import popups,{Data} from './components/pop-ups.vue'
// 骨架屏
