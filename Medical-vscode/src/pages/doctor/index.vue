<template>
  <view class="doctor-top filter-view">
    <view class="all-text" :class="selectIndex == -1 ? 'changeColor' : ''" @click="allDoctor">全部</view>
      <scroll-view scroll-x="true" class="scroll-view_H"> 
        <view class="week" v-for="(item, index) in doctor_time" :key="index" @click="selectDoctor(index, item.dep_id, item.date)">
          <text>{{item.date}}</text>
  ...
  <!-- 上面这一句给出一个高度，分为上下两个部分，上：时间，下：医生 -->
  <!-- 医生列表 -->
  <view class="doctor-list" v-for="(item, index) in doctor_list" :key="index" @click="jumpRoute(item._id)">
    ...
    <view class="doctor-name">
      ...
      <!-- <text>韩巍</text>
      <text>{{item.post}}</text>
      <text>擅长：创伤骨科各病种的诊断及常规治疗，熟悉四肢及盆骨髋臼骨折的微创治疗</text> -->
    </view>
...
onLoad(async(event:any)=>{
  ...
  dep_id.value = id
  //Promise.all([ , ])并发两个接口
  // const res1 = await RequestApi.TimeSele({dep_id:"1658914647275"}) 
  // console.log("res1我加的",res1)
  // const res2 = await RequestApi.AlldList({dep_id:"1658914647275"}) 
  // console.log("res2我加的",res2)
  // const res = await Promise.all([RequestApi.TimeSele({dep_id:id}),RequestApi.AlldList({dep_id:id})]) as ResData[]
  // console.log("res添加前",res)
  await Promise.all([
  uni.request({
    ...
    success: res=> {
        ...
        //console.log("doctor_time",doctor_time)
    }
})
    ,
    uni.request({
...
// 根据日期筛选医生列表
async function selectDoctor(index:number,dep_id:string,date:string){
