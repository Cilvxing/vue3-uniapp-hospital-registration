...
function flushPreFlushCbs(seen, parentJob = null) {
  if (pendingPreFlushCbs.length) {
    ...
    for (preFlushIndex = 0; preFlushIndex < activePreFlushCbs.length; preFlushIndex++) {
      if ({}.NODE_ENV !== "production" && checkRecursiveUpdates(seen, activePreFlushCbs[preFlushIndex])) {
        continue;
      }
...
const PublicInstanceProxyHandlers = {
  get({ _: instance }, key) {
    ...
    if ({}.NODE_ENV !== "production" && setupState !== EMPTY_OBJ && setupState.__isScriptSetup && hasOwn(setupState, key)) {
  ...
  set({ _: instance }, key, value) {
    ...
    if (setupState !== EMPTY_OBJ && hasOwn(setupState, key)) {
      ...
      ({}).NODE_ENV !== "production" && warn$1(`Attempting to mutate prop "${key}". Props are readonly.`, instance);
      return false;
    }
    if (key[0] === "$" && key.slice(1) in instance) {
      ({}).NODE_ENV !== "production" && warn$1(`Attempting to mutate public property "${key}". Properties starting with $ are reserved and readonly.`, instance);
      return false;
    } else {
...
function updateProps(instance, rawProps, rawPrevProps, optimized) {
  ...
  if (!({}.NODE_ENV !== "production" && (instance.type.__hmrId || instance.parent && instance.parent.type.__hmrId)) && (optimized || patchFlag > 0) && !(patchFlag & 16)) {
    if (patchFlag & 8) {
...
function defineStore(idOrOptions, setup, setupOptions) {
  ...
  function useStore(pinia, hot) {
    ...
    pinia = ({}.NODE_ENV === "test" && activePinia && activePinia._testing ? null : pinia) || currentInstance2 && inject(piniaSymbol);
    if (pinia)
      setActivePinia(pinia);
